...
class FpgaScheduler(object):
    ...
    def initiate_node_status(self):
        ...
        for index, line in enumerate(lines):
            ...
            node_id = index
            node_ip = node_info[0]
            node_url = "http://"+node_ip+":8000"
            pcie_bw = float(node_info[1])
            if_fpga_available = int(node_info[2])
            section_num = int(node_info[3])
            roce_bw = float(node_info[4]*1024)
            self.node_list[node_ip] = PowerNode(node_id, node_ip, node_url, pcie_bw, if_fpga_available, section_num, roce_bw)
    ...
    def conduct_absolute_locality_scheduling(self, job_id, event_type):
    ...
    def launch_new_job(self, current_job_id):
	    ...
	    in_buffer_size = self.job_list[current_job_id].job_in_buf_size
	    out_buffer_size = self.job_list[current_job_id].job_out_buf_size
	    acc_name = self.job_list[current_job_id].job_acc_name
	...
	def on_receive_request(self, job_node_ip, real_in_buffer_size, in_buffer_size, out_buffer_size, acc_name):
		mutex.acquire()
		self.current_job_count+=1
		mutex.release()
		current_job_id = str(self.current_job_count)
		job_arrival_time = 1000*(time.time()-self.epoch_time)
        print "[job %r] ARRIVES at time %r" %(current_job_id,job_arrival_time,)
        job_acc_bw = self.acc_type_list[acc_name]
        job_execution_time = (10 ** 6)*float(real_in_buffer_size)/float(job_acc_bw)/( (10**3)*(2**10))
    ...
    def update_job_info(self, current_job_id, job_open_time, job_real_execution_time, job_total_time, job_complete_time):
	...
	job_complete_time = 1000*(time.time() - self.epoch_time)
        print "[job%r] COMPLETES at time %r" %(job_id,job_complete_time,)
	print "[job%r] OPEN: %r, EXECUTE: %r, total RESPONSE: %r" %(job_id, job_open_time, job_real_execution_time, job_total_time)
	print "[job%r] IDEAL EXECUTION: %r" %(job_id, self.job_list[job_id].job_execution_time,)
	print ""
        ...
        self.update_job_info(current_job_id, job_open_time, job_real_execution_time, job_total_time, job_complete_time)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print "Usage: ",sys.argv[0], " <IPADDR>  <PORT>  <ALGORITHM>"
		...
		elif sys.argv[3] == "LOCALITY1":
			scheduling_algorithm = "LOCALITY1"
		elif sys.argv[3] == "LOCALITY2":
			scheduling_algorithm = "LOCALITY2"
