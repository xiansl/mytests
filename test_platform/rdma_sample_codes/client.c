...
int main(int argc, char **argv)
{
    ...
    if (argc != 5)
        die("usage: client <server-address> <server-port> <send_buffer_size> <recv_buffer_size>");
        send_buffer_size = atoi(argv[3]);
        recv_buffer_size = atoi(argv[4]);
...
void client_write(uint32_t len){
    ...
    printf("wait for client write completion\n");
    poll_cq(NULL);//wait for write completion
    printf("wait for server write compeltion\n");
